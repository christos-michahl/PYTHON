...
#temporary limits to image size for  y->starty<y<endy
#this will be removed for the finel version of the code
#and the image size will be the full image
starty=1300
endy=2250
	...
	if erflag==0:
		...
		for y in range(y1, y2):
			for x in range(x1, x2):
				...
				if failure==0:
					if rank==0:
						print("height for ",vp[0],",",vp[1],"not available. The cell is skipped")
					#print(vp[0],vp[1])
					continue
			
				if rank==0:

					print("calculating viewshed for",vp[0],",",vp[1])

				viewshed(vp,data,startx,endx,starty,endy)
	elif erflag==1:
...
def	viewshed(vp,data,startx,endx,starty,endy):
	...
	if rank!=0 or size==1:
		...
		height1= []
		height2 =[]
		#print("na to dist",len(currentdist1))
	...
	if rank==0 :
		...
		img = Image.new( 'RGB', (3200,3200), "black")
		pixels = img.load()
	...
	#	pass
		...
		with rasterio.open('DSM_Test.tif', 'r+') as src:		
